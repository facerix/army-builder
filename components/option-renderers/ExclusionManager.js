 ...
 */
export const getExcludedItemsPresent = (excludes, allDefaultItems, allCurrentOptions, allAvailableOptions) => {
  if (!excludes || !Array.isArray(excludes)) return [];
  
  const presentExcludedItems = [];
  const defaultItemNames = allDefaultItems.map(item => typeof item === 'string' ? item : item.name);
    ...
    if (defaultItemNames.includes(excludedName)) {
      ...
      for (const checkType of ["weapons", "wargear"]) {
        ...
        const hasReplacementSelected = availableOptions.some(availableOpt => {
          ...
          const currentOpt = allCurrentOptions[checkType]?.find(opt => matchOptionsByName(availableOpt, opt));
          const isSelected = currentOpt && currentOpt.selected && currentOpt.selected !== false && currentOpt.selected !== "off";
          
          if (!isSelected) return false; // Option not selected, so it doesn't replace anything
          
          const replacesArray = Array.isArray(availableOpt.replaces) ? availableOpt.replaces : [availableOpt.replaces];
 ...
 */
export const isExcludedItemPresent = (excludes, allDefaultItems, allCurrentOptions, allAvailableOptions, optionType) => {
  const presentItems = getExcludedItemsPresent(excludes, allDefaultItems, allCurrentOptions, allAvailableOptions);
