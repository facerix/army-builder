...
import { optionFieldsetName, findCurrentOption, calculateEffectiveMax, validateOption } from './OptionUtils.js';
 ...
 */
export const renderOption = (option, currentOptions, optionsKey, unitSize, allDefaultItems, allCurrentOptions, allAvailableOptions) => {
  // Validate option
  ...
  const effectiveMax = normalizedOption.max !== undefined ? calculateEffectiveMax(normalizedOption, unitSize) : 0;
  ...
  if (normalizedOption.max !== undefined) {
    ...
    if (Array.isArray(normalizedOption.name) && normalizedOption.max === 1 && normalizedOption.replaces) {
      return renderArrayMaxOneWithReplaces(normalizedOption, currentOption, fieldsetName, isDisabled);
    }
    ...
    if (Array.isArray(normalizedOption.name) && normalizedOption.selectionType) {
      switch (normalizedOption.selectionType) {
        case "all":
          return renderSelectionTypeAll(normalizedOption, currentOption, fieldsetName, effectiveMax, isDisabled);
        case "any":
          return renderSelectionTypeAny(normalizedOption, currentOption, fieldsetName, effectiveMax, isDisabled);
        case "anyNoDuplicates":
          return renderSelectionTypeAnyNoDuplicates(normalizedOption, currentOption, fieldsetName, effectiveMax, isDisabled);
        default:
    ...
    if (Array.isArray(normalizedOption.name)) {
      if (normalizedOption.max === 1) {
        if (normalizedOption.replaces) {
          ...
          return renderArrayMaxOneWithoutReplaces(normalizedOption, currentOption, fieldsetName, isDisabled);
        }
      } else {
    ...
    if (typeof normalizedOption.name === 'string') {
      if (normalizedOption.per) {
        return renderSingleStringWithPer(normalizedOption, currentOption, fieldsetName, effectiveMax, isDisabled);
      } else {
        return renderSingleStringWithMax(normalizedOption, currentOption, fieldsetName, isDisabled, exclusionReason);
      }
    }
  }
